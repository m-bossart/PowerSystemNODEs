...
mutable struct NODETrainParams
    ...
    batch_seed::Int64     #re-create results?  
...
function train(params::NODETrainParams, data::TrainData)
    ...
    !(params.optimizer_adjust == "nothing") && (optimizer_adjust = instantiate_optimizer_adjust(params))

    #READ DATA AND INPUT SYSTEM 
    df = DataFrame(Arrow.Table(data.faults_results_dir))
    pvss = collect(get_components(PeriodicVariableSource, data.sys))

    #TRAIN SEQUENTIALLY 
    local min_θ, res, output_data
    output_data=[] 
    min_θ = initial_params(nn)
    for pvs in pvss 
        @show min_θ[1]
        res, output_data = train(min_θ, params, sensealg, solver, optimizer, nn, M, df, pvs, output_data)
        min_θ = copy(res.u)
    end 
...
function train(θ, params, sensealg, solver, optimizer, nn, M, df, pvs, output_data)
    ...
    #DEFINE SCALE FACTORS FOR THE CURRENT PVS 
    Vr_scale = 1/(maximum(Vm.(tsteps).*cos.(Vθ.(tsteps))) - minimum(Vm.(tsteps).*cos.(Vθ.(tsteps))))
    Vi_scale = 1/(maximum(Vm.(tsteps).*sin.(Vθ.(tsteps))) - minimum(Vm.(tsteps).*sin.(Vθ.(tsteps))))

    #FIND INITIAL CONDITIONS FOR THE SURROGATE, BUILD AND SOLVE THE PROBLEM, AND CONFIRM IT MATCHES THE VERIFICATION DATA PROVIDED.
    ...
    tspan = (tsteps[1], tsteps[end])   #tspan comes from data 
    surr_func = ODEFunction(surr, mass_matrix = M)
    surr_prob = ODEProblem(surr_func, x₀_surr, tspan, p)
    sol = solve(surr_prob, solver, abstol=params.solver_tols[1], reltol=params.solver_tols[2], saveat=tsteps )
    ...
    pred_function = instantiate_pred_function(p_fixed, solver, surr_prob, params.solver_tols, sensealg, u₀) 
    loss_function = instantiate_loss_function(params.loss_function_weights, Ir_scale, Ii_scale, pred_function) 
    ...
    try
        ...
        for range in ranges
            ...
            i_curr = i_true[:,range]
            t_curr = tsteps[range]
            train_loader = Flux.Data.DataLoader((i_curr,t_curr), batchsize=Int(floor(length(i_curr[1,:]))))     #TODO - IMPLEMENT BATCHING
            optfun = OptimizationFunction((θ, p, batch, time_batch) -> loss_function(θ, batch, time_batch), GalacticOptim.AutoForwardDiff())
            optprob = OptimizationProblem(optfun, min_θ)

            cb = instantiate_cb!(output_data, params.lb_loss, params.export_mode, id, range_count)   
            range_count += 1 

            res = GalacticOptim.solve(optprob, optimizer, ncycle(train_loader, params.maxiters), cb=cb)
            min_θ = copy(res.u)
            @show min_θ[1]
