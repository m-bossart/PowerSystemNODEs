...
function _loss_function(θ, y_actual, tsteps,  weights, Ir_scale, Ii_scale, pred_function)
    ...
    loss = 
        (mae(y_predicted[1,:], y_actual[1,:]) / Ir_scale) + (mae(y_predicted[2,:], y_actual[2,:]) / Ii_scale) * weights[1] + 
        (mse(y_predicted[1,:], y_actual[1,:]) / Ir_scale) + (mse(y_predicted[2,:], y_actual[2,:]) / Ii_scale) * weights[2]  
    return loss, y_predicted
end

function instantiate_loss_function( weights, Ir_scale, Ii_scale, pred_function)
    return (θ, y_actual, tsteps) -> _loss_function(θ,  y_actual, tsteps, weights, Ir_scale, Ii_scale, pred_function)
end

function _pred_function(θ, tsteps, p_fixed, solver, surr_prob, tols,  sensealg, u₀)
    p = vcat(θ,  p_fixed) 
    _prob = remake(surr_prob, p=p, u0=u₀)    
    sol = solve(_prob, solver,  abstol=tols[1], reltol=tols[2], saveat=tsteps, 
...
function instantiate_pred_function(p_fixed, solver, surr_prob, tols, sensealg, u₀)
    return (θ, tsteps) -> _pred_function(θ, tsteps, p_fixed, solver, surr_prob, tols, sensealg, u₀)
end
