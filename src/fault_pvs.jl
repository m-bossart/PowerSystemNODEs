    ...
    for (i,fault) in enumerate(faults)  
        ...
        if OutputParameters["OutputData"]["BusNumbers"] !== nothing 
            for (i,bus_number) in enumerate(OutputParameters["OutputData"]["BusNumbers"])
                if OutputParameters["OutputData"]["BusData"][i] == "Vm"
                    ...
                    timeseries_data = hcat(timeseries_data, get_voltage_magnitude_series(results, bus_number)[2])
                elseif OutputParameters["OutputData"]["BusData"][i] == "Vtheta"
                    push!(column_names, string(  bus_number, "_Vtheta"))
                    if bus_number == pvs_bus_number
                        pvs_data["Vθ_index"] = i+1
                    end
                    timeseries_data = hcat(timeseries_data, get_voltage_angle_series(results, bus_number)[2])
                else 
        ...
        if OutputParameters["OutputData"]["DynamicDevices"] !== nothing 
            for (i,device_name) in enumerate(OutputParameters["OutputData"]["DynamicDevices"])
                ...
                timeseries_data = hcat(timeseries_data, get_state_series(results, (device_name, state_symbol))[2])
            end
        end 
        d = Dict("fault" => fault, "data"=>DataFrame(timeseries_data,Symbol.(column_names))) 
...
function get_fault(fault_device_name, fault_type, system, t_fault)
    ...
    elseif fault_type[1] == "ControlReferenceChange"
        ...
        fault = ControlReferenceChange(t_fault, g, :P_ref, starting_Pref*fault_type[2]["RefValue"])
    ...
    elseif fault_type[1] == "LoadChange"
        ...
        fault = PowerSimulationsDynamics.LoadChange(t_fault, l, :P_ref, starting_Pref* fault_type[2]["RefValue"])
