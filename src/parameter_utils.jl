...
function randomize_parameters!(inv::DynamicInverter, param_range::Tuple{Float64, Float64})
    ...
    set_ω_lp!(
        inv.freq_estimator,
        get_ω_lp(inv.freq_estimator) * rand(Uniform(param_range[1], param_range[2])),
    )
    set_kp_pll!(
        inv.freq_estimator,
        get_kp_pll(inv.freq_estimator) * rand(Uniform(param_range[1], param_range[2])),
    )
    set_ki_pll!(
        inv.freq_estimator,
        get_ki_pll(inv.freq_estimator) * rand(Uniform(param_range[1], param_range[2])),
    )
    #outer control
    PSY.set_Ta!(
        inv.outer_control.active_power,
        PSY.get_Ta(inv.outer_control.active_power) *
        rand(Uniform(param_range[1], param_range[2])),
    )
    PSY.set_kd!(
        inv.outer_control.active_power,
        PSY.get_kd(inv.outer_control.active_power) *
        rand(Uniform(param_range[1], param_range[2])),
    )
    PSY.set_kω!(
        inv.outer_control.active_power,
        PSY.get_kω(inv.outer_control.active_power) *
        rand(Uniform(param_range[1], param_range[2])),
    )
    PSY.set_kq!(
        inv.outer_control.reactive_power,
        PSY.get_kq(inv.outer_control.reactive_power) *
        rand(Uniform(param_range[1], param_range[2])),
    )
    PSY.set_ωf!(
        inv.outer_control.reactive_power,
        PSY.get_ωf(inv.outer_control.reactive_power) *
        rand(Uniform(param_range[1], param_range[2])),
    )
    #inner control
    set_kpv!(
        inv.inner_control,
        get_kpv(inv.inner_control) * rand(Uniform(param_range[1], param_range[2])),
    )
    set_kiv!(
        inv.inner_control,
        get_kiv(inv.inner_control) * rand(Uniform(param_range[1], param_range[2])),
    )
    set_kffv!(
        inv.inner_control,
        get_kffv(inv.inner_control) * rand(Uniform(param_range[1], param_range[2])),
    )
    set_rv!(
        inv.inner_control,
        get_rv(inv.inner_control) * rand(Uniform(param_range[1], param_range[2])),
    )
    set_lv!(
        inv.inner_control,
        get_lv(inv.inner_control) * rand(Uniform(param_range[1], param_range[2])),
    )
    set_kpc!(
        inv.inner_control,
        get_kpc(inv.inner_control) * rand(Uniform(param_range[1], param_range[2])),
    )
    set_kic!(
        inv.inner_control,
        get_kic(inv.inner_control) * rand(Uniform(param_range[1], param_range[2])),
    )
    set_kffi!(
        inv.inner_control,
        get_kffi(inv.inner_control) * rand(Uniform(param_range[1], param_range[2])),
    )
    set_ωad!(
        inv.inner_control,
        get_ωad(inv.inner_control) * rand(Uniform(param_range[1], param_range[2])),
    )
    set_kad!(
        inv.inner_control,
        get_kad(inv.inner_control) * rand(Uniform(param_range[1], param_range[2])),
    )
    #lcl
