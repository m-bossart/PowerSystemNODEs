...
p2a = plot(tsteps, ode_data[1,:], xlabel=L"t \: \mathrm{[s]}", ylabel=L"i^r_{tvib} \: \mathrm{[pu]}", label="full order model", width=1.5)
plot!(p2a, tsteps, avgmodel_data[1,:], width = 1.5,  label="average model" )
p2b = plot(tsteps, ode_data[1,:], xlabel=L"t \: \mathrm{[s]}", ylabel=L"i^r_{tvib} \: \mathrm{[pu]}",label="full order model", width=1.5, xscale=:log)
plot!(p2b, tsteps, avgmodel_data[1,:], width = 1.5,  label="average model" )
p2c = plot(tsteps, ode_data[2,:], xlabel=L"t \: \mathrm{[s]}", ylabel=L"i^i_{tvib} \: \mathrm{[pu]}",label="full order model", width=1.5)
plot!(p2c, tsteps, avgmodel_data[2,:], width = 1.5,  label="average model" )
p2d = plot(tsteps, ode_data[2,:], xlabel=L"t \: \mathrm{[s]}", ylabel=L"i^i_{tvib} \: \mathrm{[pu]}",label="full order model", width=1.5, xscale=:log)
...
p3c = plot(data[:,1], data[:,3], xlabel=L"t \: \mathrm{[s]}", ylabel=L"i^r_{tvib} \: \mathrm{[pu]}", width=1.5, label="true solution",xscale=:log)
plot!(p3c, data[:,1], data[:,5], label="UODE model")
plot!(p3c, data[:,1], data[:,11], label="average inverter model")
p3d = plot(data[:,1], data[:,9],  xlabel=L"t \: \mathrm{[s]}", ylabel=L"i^r_{nn} \: \mathrm{[pu]}", width=1.5,xscale=:log)
