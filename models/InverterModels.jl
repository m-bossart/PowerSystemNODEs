...
#19th order gfm model with nn current source on the output with input V(t)
#and output I(t)
function gfm_nn_voltage(dx,x,p,t)
    #PARAMETERS
    p_nn = p[1:n_weights_nn_voltage]
    p_ode = p[n_weights_nn_voltage+1:end]
    ...
    dx[i__ii_filter]= +(ω_base/lg) *                                            #docs:(5f)
     ...
     dx[i__ir_nn] =ir_offset -  nn_voltage([Vr(t), Vi(t)],p_nn)[1]
     dx[i__ii_nn] =ii_offset -  nn_voltage([Vr(t), Vi(t)],p_nn)[2]
...
#21 order model which includes the GFM model + 2 algebraic equations for line to IB
#Preferably will avoid using this formulation, but left below for reference.
#If you have a singled device + IB in PSID and you need a non-zero impedance
#between the two we can always split the filter inductor into two parts.
