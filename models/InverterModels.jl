...
#and output I(t)
function gfm_nn_out(dx,x,p,t)
    #PARAMETERS
    p_nn = p[1:n_weights]
    p_ode = p[n_weights+1:end]
    ...
    lg = p_ode[22]
    rg = p_ode[23]
    Vref = p_ode[24]    #Reference treated as parameter
    ωref = p_ode[25]    #Reference treated as parameter
    Pref = p_ode[26]    #Reference treated as parameter
    Qref = p_ode[27]    #Reference treated as parameter
    ...
    dx[i__ii_filter]= +(ω_base/lg) *                       #docs:(5f)
     ...
     #dx[i__ir_nn] = nn([Vr(t),Vi(t)],p_nn)[1] - 1.0 * ir_nn   #CR: add integrator of PLL state to the NN? ( will be 0 in steady state!)
     #dx[i__ii_nn] = nn([Vr(t),Vi(t)],p_nn)[1] - 1.0 * ii_nn

     #Can't use PSID to initailize
     dx[i__ir_nn] = nn(x,p_nn)[1] - 1.0 * ir_nn   #CR: add integrator of PLL state to the NN? ( will be 0 in steady state!)
     dx[i__ii_nn] = nn(x,p_nn)[1] - 1.0 * ii_nn


end

#19th order gfm model with nn terms distributed across all blocks .
function gfm_nn_dist(dx,x,p,t)
    #PARAMETERS
    p_nn = p[1:n_weights]
    p_ode = p[n_weights+1:end]
    ...
    i__vq_pll, vq_pll = 3, x[3]
    i__γq_ic, γq_ic = 4, x[4]
    i__ir_filter, ir_filter = 5, x[5] + nn(x,p_nn)[1]   #NN is a current source before the filter inductor
    i__ξd_ic, ξd_ic = 6, x[6]
    i__ϕd_ic, ϕd_ic = 7, x[7]
    i__ε_pll, ε_pll = 8, x[8]
    i__ir_cnv, ir_cnv = 9, x[9]
    ...
    i__θ_oc, θ_oc = 17, x[17]
    i__ii_cnv, ii_cnv = 18, x[18]
    i__ii_filter, ii_filter = 19, x[19] + nn(x,p_nn)[2]  #NN is a current source before the filter inductor
...
#21 order model which includes the GFM model + 2 algebraic equations
function gfm_ib(dx,x,p,t)
    #STATE INDEX AND STATES
    i__vi_filter, vi_filter = 1, x[1]
    i__γd_ic, γd_ic = 2, x[2]
    i__vq_pll, vq_pll = 3, x[3]
