#Naming of surrogate models is as follows: <analytical_model>_nn_<nn_external_inputs>_<#_feedback_states>
...
function vsm_nn_v_2(dx,x,p,t)
    #PARAMETERS
    p_nn = p[1:n_weights_nn]  #n_weights_nn must be defined in global scope prior to training
...
function vsm_nn_v_3(dx,x,p,t)
  ...
  #NN INPUT 
  nn_input = [(Vr_pcc - Vr0)*Vr_scale, (Vi_pcc - Vi0)*Vi_scale, ir_nn, ii_nn, nn3]
  ...
  dx[i__ii_nn] = nn(nn_input, p_nn)[2] * nn_scale
   ...
   #ADDITIONAL NN STATES 
   dx[i__nn3] = nn(nn_input, p_nn)[3] * nn_scale
end

function vsm_nn_v_4(dx,x,p,t)
  #PARAMETERS
  p_nn = p[1:n_weights_nn]  #n_weights_nn must be defined in global scope prior to training
  p_ode = p[n_weights_nn+1:end]
  ...
  #NN INPUT 
  nn_input = [(Vr_pcc - Vr0)*Vr_scale, (Vi_pcc - Vi0)*Vi_scale, ir_nn, ii_nn, nn3, nn4]
  ...
  dx[i__ii_nn] = nn(nn_input, p_nn)[2] * nn_scale
   ...
   #ADDITIONAL NN STATES 
   dx[i__nn3] = nn(nn_input, p_nn)[3] * nn_scale
   dx[i__nn4] = nn(nn_input, p_nn)[4] * nn_scale
end

function vsm_nn_v_5(dx,x,p,t)
  #PARAMETERS
  p_nn = p[1:n_weights_nn]  #n_weights_nn must be defined in global scope prior to training
  p_ode = p[n_weights_nn+1:end]
  ...
  #NN INPUT 
  nn_input = [(Vr_pcc - Vr0)*Vr_scale, (Vi_pcc - Vi0)*Vi_scale, ir_nn, ii_nn, nn3, nn4, nn5]
  ...
  dx[i__ii_nn] = nn(nn_input, p_nn)[2] * nn_scale
   ...
   dx[i__nn3] = nn(nn_input, p_nn)[3] * nn_scale
   dx[i__nn4] = nn(nn_input, p_nn)[4] * nn_scale
   dx[i__nn5] = nn(nn_input, p_nn)[5] * nn_scale
end