...
function cb_plot(pred, plot_log::Bool)
    if plot_log
        ...
        p1_log =  plot(tsteps_train, pred[1,:], markersize=2, label = "real current prediction", xaxis=:log)
        plot!(p1_log, tsteps, ode_data[1,:],   label = "real current true")
        plot!(p1_log, tsteps, avgmodel_data[1,:], label = "real current avg. model")
        p2 = plot(tsteps_train, pred[2,:], markersize=2, label = "imag current prediction")
        plot!(p2, tsteps, ode_data[2,:],  label = "imag current true")
        plot!(p2, tsteps, avgmodel_data[2,:], label = "imag current avg. model")
        p2_log = plot(tsteps_train, pred[2,:], markersize=2, label = "imag current prediction")
        ...
        p3 = plot(tsteps_train, pred[3,:], markersize=2, label = "real current from inverter")
        p3_log = plot(tsteps_train, pred[3,:], markersize=2, label = "real current from inverter", xaxis=:log)
        p4 = plot(tsteps_train, pred[4,:], markersize=2, label = "imag current from inverter")
        p4_log = plot(tsteps_train, pred[4,:], markersize=2, label = "imag current from inverter", xaxis=:log)

        p5 = plot(tsteps_train, pred[5,:], markersize=2, label = "real current from nn source")
        p5_log = plot(tsteps_train, pred[5,:], markersize=2, label = "real current from nn source", xaxis=:log)
        p6 = plot(tsteps_train, pred[6,:], markersize=2, label = "imag current from nn source")
        p6_log = plot(tsteps_train, pred[6,:], markersize=2, label = "imag current from nn source", xaxis=:log)
